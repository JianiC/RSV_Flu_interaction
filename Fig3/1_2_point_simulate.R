simulate_tss <- function(params, res_hhs=res_hhs8_arsv_neutral, give_everything = FALSE, show_progress = TRUE,...) {
  # browser()
...
point_params<-data.frame(psi=c(0.05,0.25,1,1,0.5,0.5),chi=c(0.1,0.25,1,0.5,1,0.5))
pointparams_design<-point_params%>% bind_cols(ref_df)


pointsim_result<- map_df(1:6, multi_simulate_tss, params = pointparams_design, 
                       give_everything = TRUE, show_progress = FALSE)


p_label <- tibble(
  facet_label = c("a", "b", "c","d", "e", "f"),
  x_cord = 2013,
  y_cord = 250, 
  label = c(expression(paste(psi==0.05,",", chi==0.1)),expression(paste(psi==0.25,",", chi==0.25)),
            expression(paste(psi==1,",", chi==1)),expression(paste(psi==1,",", chi==0.5)),
            expression(paste(psi==0.5,",", chi==0.5)),expression(paste(psi==0.5,",", chi==0.5))
          ) 
          )
...
pointsim_result%>%
  ...
  mutate(facet_label = rep(c("a", "b", "c","d", "e", "f"), each = length(seq(0.5, 3.5-1/52, by = 1/52))))%>%
...
facet_result%>%
  ...
  geom_text(data= p_label,aes(x = 2013.5, y =300 ,label = facet_label),inherit.aes = FALSE,parse = T,size=4)+
  geom_text(data= p_label,aes(x = 2014.2, y =300 ,label = label),inherit.aes = FALSE,parse = T,size=4)+
  labs(x = "Time (weeks) ", 
       y = "Cases / million") +
  ...
  theme(  strip.background = element_blank(),
                    ...
                    legend.position = "top",
                    text = element_text(size = 12),        axis.title = element_text(size = 12),   # Increases axis labels
          axis.text = element_text(size = 12),    # Increases axis ticks
          legend.text = element_text(size =12)) ->point_simu
...
ggsave(simu_plot, file = "fig3.pdf", width = 8.5, height = 9.5, units = "in")
